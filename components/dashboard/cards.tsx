...
import type {
  AgentStatusSnapshot, MarketSignal, NewsItem,
  PolymarketSnapshot, DecisionLog,
} from "@/types";
import {
  formatCurrency, formatNumber, trendColor, timeAgo,
  riskColor, signalColor, decisionColor,
} from "@/lib/utils";

// ─── Agent Status ──────────────────────────────────────────────

export function AgentStatusCard({ status = mockAgentStatus }: { status?: AgentStatusSnapshot }) {
  return (
    <Card glow="gold" className="col-span-full md:col-span-2">
      <CardHeader
        ...
        action={<span className="text-xs font-mono text-aurum-text-dim">{timeAgo(status.snapshotAt)}</span>}
      />
      <div className="flex items-start gap-6">
        <div className="flex-1 space-y-4">
          <DecisionBadge decision={status.decision} />
          <ConfidenceBar value={status.confidence} label="Confidence" showValue />
          <p className="text-sm text-aurum-text-secondary leading-relaxed">{status.reasoning}</p>
        </div>
        <div className="hidden md:block w-px bg-aurum-bg-border self-stretch" />
        <div className="hidden md:block space-y-3 min-w-[140px]">
          <SignalRow label="Risk Level" value={status.riskLevel.toUpperCase()} color={riskColor(status.riskLevel)} mono />
          <SignalRow label="Composite" value={`${status.compositeScore > 0 ? "+" : ""}${status.compositeScore}`}
            color={status.compositeScore > 0 ? "text-emerald-400" : status.compositeScore < 0 ? "text-rose-400" : "text-aurum-text-secondary"} mono />
          <SignalRow label="Treasury" value={`${status.treasuryHealth}%`} color="text-emerald-400" mono />
          <SignalRow label="Runway" value={`${status.computeRunway}d`} color="text-aurum-gold" mono />
        </div>
      </div>
    </Card>
  );
...
export function CompositeSignalCard({ status = mockAgentStatus }: { status?: AgentStatusSnapshot }) {
  const score = status.compositeScore;
  ...
  return (
    <Card glow="purple">
      ...
      <div className="space-y-4">
        ...
        <ConfidenceBar value={confidence} label="Confidence" showValue />
        <div className="pt-3 border-t border-aurum-bg-border space-y-1">
          <SignalRow label="Chart" value={status.chartSignal?.toUpperCase() ?? "—"} mono />
          <SignalRow label="News" value={status.newsSignal?.toUpperCase() ?? "—"} mono />
          <SignalRow label="Polymarket" value={status.polymarketSignal?.toUpperCase() ?? "—"} mono />
          <SignalRow label="Risk Budget" value={`${status.treasuryHealth}%`} color="text-emerald-400" mono />
        </div>
      </div>
    </Card>
  );
...
export function MarketOverviewCard({ signals = mockMarketSignals }: { signals?: MarketSignal[] }) {
  return (
    <Card>
            ...
            <div key={sig.id} className="flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs font-mono ${trendColor(sig.trend)}`}>{sig.trend.toUpperCase()}</span>
                </div>
                <p className="text-xs text-aurum-text-dim line-clamp-1">{sig.summary}</p>
              </div>
              <div className="w-20 flex-shrink-0">
                <Sparkline data={bars.slice(-24)}
...
export function ChartAnalysisCard({ signals = mockMarketSignals }: { signals?: MarketSignal[] }) {
  const btc = signals[0] ?? mockMarketSignals[0];
  return (
    <Card>
      <CardHeader title="Chart Analysis" icon={<BarChart2 className="w-4 h-4" />}
      ...
      <div className="space-y-1">
        ...
        <SignalRow label="Trend" value={btc.trend.toUpperCase()} color={trendColor(btc.trend)} mono />
        <SignalRow label="Confidence" value={`${btc.confidence}%`} mono />
        <SignalRow label="Support" value={btc.keyLevels?.support?.map((l: number) => l.toLocaleString()).join(" / ") ?? "—"} mono />
        <SignalRow label="Resistance" value={btc.keyLevels?.resistance?.map((l: number) => l.toLocaleString()).join(" / ") ?? "—"} mono />
      </div>
...
export function NewsPulseCard({ news = mockNewsItems }: { news?: NewsItem[] }) {
  return (
    <Card>
          ...
          return (
            <div key={item.id} className="flex items-start gap-3 py-2 border-b border-aurum-bg-border/40 last:border-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs text-aurum-text-secondary leading-relaxed">{item.headline}</p>
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className={`text-xs font-mono ${color} ml-auto`}>{item.relevanceScore}%</span>
                </div>
              </div>
            </div>
          );
...
export function PolymarketContextCard({ markets = mockPolymarketSnapshots }: { markets?: PolymarketSnapshot[] }) {
  return (
    <Card glow="gold">
        ...
        {markets.map((m) => (
          <div key={m.id} className="space-y-2">
            <div className="flex justify-between items-start gap-4">
              <p className="text-xs text-aurum-text-secondary leading-relaxed flex-1">{m.question}</p>
              <div className="text-right flex-shrink-0">
                <p className="text-sm font-mono text-aurum-gold">{(m.yesOdds * 100).toFixed(0)}%</p>
            ...
            <div className="w-full bg-aurum-bg-border rounded-full h-1 overflow-hidden">
              <div className="h-full bg-aurum-gold/60 rounded-full" style={{ width: `${m.yesOdds * 100}%` }} />
            </div>
...
export function DecisionFeedCard({ decisions = mockDecisionLog }: { decisions?: DecisionLog[] }) {
  return (
        ...
        {decisions.map((dec) => (
          <div key={dec.id} className="flex gap-4 p-4 rounded-md border border-aurum-bg-border/60 bg-aurum-bg-elevated/40">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-aurum-text-secondary leading-relaxed mb-2">{dec.reasoning}</p>
              {dec.outcome && (
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Stat label="Fees 24H" value={formatCurrency(tok.feesCollected24h)} change="+12.4%" changePositive mono />
...
export function RecentActionsCard() {
  const actions = [
    ...
    { time: "15m ago", action: "Signal synthesis", status: "ok", detail: "Composite signal generated" },
      ...
      <div className="space-y-2">
        {actions.map((a, i) => (
          <div key={i} className="flex items-center gap-3 py-1.5">
            <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${a.status === "ok" ? "bg-emerald-400" : "bg-aurum-text-dim"}`} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
