...
import type { AgentStatusSnapshot, NewsItem, PolymarketSnapshot } from "@/types";
    ...
    <section className="py-32 border-t border-aurum-bg-border/30">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          >
            <p className="text-xs font-mono tracking-[0.2em] uppercase text-aurum-text-dim mb-4">Public Intelligence</p>
          ...
          >
            ...
            <div className="rounded-lg border border-aurum-bg-border bg-aurum-bg-card p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono tracking-[0.12em] uppercase text-aurum-text-dim">Agent Status</span>
                </div>
                <DecisionBadge decision={status.decision} />
              </div>
              <ConfidenceBar value={status.confidence} label="Confidence" showValue />
              <p className="text-xs text-aurum-text-secondary mt-4 leading-relaxed line-clamp-3">{status.reasoning}</p>
            </div>

            {/* News */}
            <div className="rounded-lg border border-aurum-bg-border bg-aurum-bg-card p-5">
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xs font-mono tracking-[0.12em] uppercase text-aurum-text-dim">News Pulse</span>
              </div>
                  ...
                  return (
                    <div key={item.id} className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-aurum-text-secondary line-clamp-1 leading-relaxed">{item.headline}</p>
                        <p className="text-xs text-aurum-text-dim mt-0.5">{item.source} · {timeAgo(item.publishedAt)}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="rounded-lg border border-aurum-gold/15 bg-aurum-gold/3 p-5">
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xs font-mono tracking-[0.12em] uppercase text-aurum-text-dim">Polymarket Signals</span>
              </div>
              <div className="space-y-3">
                {polymarket.slice(0, 2).map((market) => (
                  <div key={market.id}>
                    <div className="flex justify-between items-start mb-1.5">
                      <p className="text-xs text-aurum-text-secondary leading-relaxed line-clamp-2 flex-1 mr-4">{market.question}</p>
                      <span className="text-xs font-mono text-aurum-gold flex-shrink-0">{(market.yesOdds * 100).toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-aurum-bg-border rounded-full h-1 overflow-hidden">
                      <div className="h-full bg-aurum-gold/50 rounded-full" style={{ width: `${market.yesOdds * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
